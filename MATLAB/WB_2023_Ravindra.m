...
site_Q_maj_ion_data=load('Data_for_EMMA_2022_Wade.txt');
...
num_conser_solutes=6;  %----this values comes from the bivariate solute-solute plots when considering all solute pairs
 %--SO4-S (3), dD(6), d18O(7), Mg(13), Na(15), Si(18)
...
site_Q_maj_ion_data_v2(:,7)=site_Q_maj_ion_data(:,18);   
...
%---col(4): standardized  Mg (mg/L)
%---col(5): standardized  Na (mg/L)
%---col(6): standardized  Si (mg/L)

%%%%%%%%%%%%%%%%%% for conservative tracers only %%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%% solute-solute plot for only conservative solutes begins here %%%%%%%%%%%%%%%%%%%%%%
ele_int_P6=  [  2,      3,     4,    5,    6,    7 ];
% %----------   SO4-S,  dD,  d18O,  Mg,   Na,  Si------
txt_lab_02={'SO_{4}-S','dD','d18O','Mg','Na','Si'};
txt_lab_02={'SO_{4}-S','dD','d18O','Mg','Na','Si'};
...
for i5=1:eig_vals_con
    ind_c1=5;  %---for RRMSE---    
    RRMSE_mat_eig_ret(i5,(1+1):(1+num_solutes))=[eig_mat_cell{i5,5}(ind_SO4,ind_c1),eig_mat_cell{i5,5}(ind_dD,ind_c1),eig_mat_cell{i5,5}(ind_d18O,ind_c1),eig_mat_cell{i5,5}(ind_Mg,ind_c1),eig_mat_cell{i5,5}(ind_Na,ind_c1),eig_mat_cell{i5,5}(ind_Si,ind_c1)];
    
     ind_c2=2;  %---for R value --    
    R_mat(i5,(1+1):(1+num_solutes))=[eig_mat_cell{i5,5}(ind_SO4,ind_c2),eig_mat_cell{i5,5}(ind_dD,ind_c2),eig_mat_cell{i5,5}(ind_d18O,ind_c2),eig_mat_cell{i5,5}(ind_Mg,ind_c2),eig_mat_cell{i5,5}(ind_Na,ind_c2),eig_mat_cell{i5,5}(ind_Si,ind_c2)];
    
    ind_c3=3;  %---for P-value  --    
    P_val_mat(i5,(1+1):(1+num_solutes))=[eig_mat_cell{i5,5}(ind_SO4,ind_c3),eig_mat_cell{i5,5}(ind_dD,ind_c3),eig_mat_cell{i5,5}(ind_d18O,ind_c3),eig_mat_cell{i5,5}(ind_Mg,ind_c3),eig_mat_cell{i5,5}(ind_Na,ind_c3),eig_mat_cell{i5,5}(ind_Si,ind_c3)];
end
...
ylabel('RRMSE');
xticks([1,2,3,4,5,6]);
legend('SO_{4}-S','dD','d18O','Mg','Na','Si');
fig = gcf;
set(gca,'fontsize',1.0*fs_leg);
...
EMs_in_all=load('emma_Wade_all.txt');
EMs_in_1std=load('EM_all_stdev_WB.txt');
...
EMs_in_all_cons_trac=NaN(length(row_ret_EMs),1+length(cons_trac));
 ...
 %%%%%%%%%%%%%%%%%---plot obs vs. predictec concentrations ends here %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                   
  ...
  %----eigen values for U2-------------------------------------------------
           V2_Uspace_sf=eig_mat_cell_f{1,2}(2,:);
  ...
  %----eigen values for U3-------------------------------------------------
          V3_Uspace_sf=eig_mat_cell_f{3,2}(3,:);
      %%%%%%%%%%%%%%%%%%%%%%%%%   
...
EMs_f1={ 'winter_gw', 'precipitation', 'snow','snow_lysimeter'}; %-- retained end-member name
EMs_ID_f1=[101, 201,301,401,501];                                                                  %--retained end-member IDS
label_em_f1=cellstr({  'winter_gw', 'precipitation', 'snow','snow_lysimeter'}); %--retained end-member name
EMs_in_all_cons_trac_f1=NaN(length(row_ret_EMs_f1),1+length(cons_trac));
EMs_in_1std_cons_trac_f1=NaN(length(row_ret_EMs_f1),1+length(cons_trac));
  %--format of the above file is:
...
w_gw=plot(EM_U_space_mat_f1(1,1),EM_U_space_mat_f1(1,2),'d','MarkerSize',EM_size_marker,'MarkerFaceColor',[0    0.7461    1.0000],'MarkerEdgeColor','k','LineWidth',4);       %--Precip (EM1.P)
s_gw=plot(EM_U_space_mat_f1(2,1),EM_U_space_mat_f1(2,2),'d','MarkerSize',EM_size_marker,'MarkerEdgeColor','b','LineWidth',4);  
pre=plot(EM_U_space_mat_f1(3,1),EM_U_space_mat_f1(3,2),'d','MarkerSize',EM_size_marker,'MarkerEdgeColor','m','LineWidth',4);  
snow=plot(EM_U_space_mat_f1(4,1),EM_U_space_mat_f1(4,2),'d','MarkerSize',EM_size_marker,'MarkerEdgeColor','c','LineWidth',4);  
snow_lys=plot(EM_U_space_mat_f1(5,1),EM_U_space_mat_f1(5,2),'d','MarkerSize',EM_size_marker,'MarkerEdgeColor','y','LineWidth',4);  
...
% plot(EM_U_space_mat_f1(16,1),EM_U_space_mat_f1(16,2),'o','MarkerSize',EM_size_marker,'MarkerFaceColor','k','MarkerEdgeColor','k');  
  ...
  xlabel('U1');
  ylabel('U2');
  fig_7_leg1=legend([w_gw],'winter gw (EM1)');
 % fig_7_leg2=legend([s_gw],'summer gw (EM2)');
  fig_7_leg3=legend([pre],'precipitation (EM3)');
  fig_7_leg4=legend([snow],'snow (EM4)');
  fig_7_leg5=legend([snow_lys],'snow lysimeter (EM5)');
  set(fig_7_leg1,'Location','northwest');
  %set(fig_7_leg2,'Location','northwest');
  set(fig_7_leg3,'Location','northwest');
  set(fig_7_leg4,'Location','northwest');
  set(fig_7_leg5,'Location','northwest');
